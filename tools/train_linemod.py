...
sys.path.append(".")
sys.path.append("..")
from lib.ransac_voting_gpu_layer.ransac_voting_gpu import (
    ransac_voting_layer_v3,
    estimate_voting_distribution_with_mean,
    ransac_voting_layer_v5,
    ransac_motion_voting,
)
from lib.networks.model_repository import *
from lib.datasets.linemod_dataset import (
    LineModDatasetRealAug,
    ImageSizeBatchSampler,
    VotingType,
)
from lib.utils.data_utils import (
    LineModImageDB,
    OcclusionLineModImageDB,
    TruncatedLineModImageDB,
)
from lib.utils.arg_utils import args
from lib.utils.draw_utils import (
...
from lib.utils.net_utils import (
    AverageMeter,
    Recorder,
    smooth_l1_loss,
    load_model,
...
class EvalWrapper(nn.Module):
    def forward(self, seg_pred, vertex_pred, use_argmax=True, use_uncertainty=False):
        ...
        if use_argmax:
            ...
            mask = seg_pred
        if use_uncertainty:
            return ransac_voting_layer_v5(
                mask, vertex_pred, 128, inlier_thresh=0.99, max_num=100
            )
        else:
            return ransac_voting_layer_v3(
                mask, vertex_pred, 128, inlier_thresh=0.99, max_num=100
            )
...
def train_net():
    ...
    if args.test_model:
        ...
        if args.normal:
            ...
            test_set = LineModDatasetRealAug(
                test_db, cfg.LINEMOD, vote_type, augment=False, use_motion=motion_model
            )
        ...
        if args.occluded and args.linemod_cls in cfg.occ_linemod_cls_names:
            ...
            occ_test_loader = DataLoader(
                occ_test_set, batch_sampler=occ_test_batch_sampler, num_workers=0
            )
        ...
        if args.truncated:
            ...
            trun_test_loader = DataLoader(
                trun_image_set, batch_sampler=trun_test_batch_sampler, num_workers=0
            )
            prefix = "trun_test"
            val(
                net,
        ...
        image_db = LineModImageDB(
            args.linemod_cls, has_fuse_set=train_cfg["use_fuse"], has_render_set=False
        )
